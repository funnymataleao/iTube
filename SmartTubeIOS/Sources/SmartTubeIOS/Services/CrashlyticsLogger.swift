...
struct CrashlyticsLogger: Sendable {
    ...
    func notice(_ message: @autoclosure () -> String) {
        ...
        #else
        logger.notice("\(msg, privacy: .private(mask: .hash))")
        #endif
#if os(tvOS)
        #if DEBUG
    ...
    func error(_ message: @autoclosure () -> String) {
        ...
        #else
        logger.error("\(msg, privacy: .private(mask: .hash))")
        #endif
#if os(tvOS)
        #if DEBUG
        if ProcessInfo.processInfo.arguments.contains("--playback-diagnostics") {
    ...
    func debug(_ message: @autoclosure () -> String) {
        ...
        #else
        logger.debug("\(msg, privacy: .private(mask: .hash))")
        #endif
#if os(tvOS)
        #if DEBUG
        if ProcessInfo.processInfo.arguments.contains("--playback-diagnostics") {
    ...
    static func sendWrongVideoReport(
        ...
        crashlytics.log(msg)
        Logger(subsystem: appSubsystem, category: "WrongVideo").error("\(msg, privacy: .private(mask: .hash))")
        crashlytics.record(error: NSError(
