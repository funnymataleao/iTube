import SwiftUI
import SmartTubeIOSCore

#if os(tvOS)
/// A focused account feed for the personal tvOS client.
/// It deliberately exposes one YouTube browse endpoint per top-level tab so the
/// Siri Remote never has to traverse a second row of section selectors.
struct PersonalTVFeedView: View {
    private let section: BrowseSection
    private let api: InnerTubeAPI

    @State private var viewModel: BrowseViewModel
    @State private var subscriptionTopicsModel: SubscriptionTopicsViewModel
    @State private var selectedVideo: Video?
    @State private var channelDestination: ChannelDestination?
    @State private var playbackReturnVideoID: String?
    @State private var restoreFocusedVideoID: String?
    @State private var subscriptionSnapshot: [Video] = []
    @State private var lastSubscriptionAuthToken: String?
    // Start every newly-created subscriptions screen on All. Keeping this as
    // view state preserves an explicit topic selection across feed updates,
    // without leaking yesterday's filter into the next app session.
    @State private var selectedTopicRawValue = SubscriptionTopic.all.rawValue

    @Environment(AuthService.self) private var auth
    @Environment(SettingsStore.self) private var store
    @Environment(\.scenePhase) private var scenePhase

    init(sectionType: BrowseSection.SectionType, api: InnerTubeAPI) {
        let section = BrowseSection(type: sectionType)
        self.section = section
        self.api = api
        _viewModel = State(initialValue: BrowseViewModel(api: api, initialSection: section))
        _subscriptionTopicsModel = State(initialValue: SubscriptionTopicsViewModel(api: api))
    }

    var body: some View {
        Group {
            if auth.accessToken?.isEmpty != false {
                signedOutState
            } else if viewModel.isLoading && videos.isEmpty {
                ProgressView()
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
                    .accessibilityLabel("Loading \(section.title)")
            } else if let error = viewModel.error, videos.isEmpty {
                errorState(error)
            } else if videos.isEmpty {
                ContentUnavailableView(
                    "No \(section.title)",
                    systemImage: section.type == .history ? "clock" : "rectangle.stack",
                    description: Text("Refresh or try again later.")
                )
            } else {
                if section.type == .subscriptions {
                    subscriptionContent
                } else {
                    standardFeedContent
                }
            }
        }
        .toolbar(.hidden, for: .navigationBar)
        .fullScreenCover(item: $selectedVideo) { video in
            PlayerView(video: video, api: api)
        }
        .navigationDestination(item: $channelDestination) { destination in
            ChannelView(channelId: destination.channelId)
        }
        .onReceive(NotificationCenter.default.publisher(for: .openChannel)) { note in
            guard let channelId = note.userInfo?["channelId"] as? String, !channelId.isEmpty else { return }
            channelDestination = ChannelDestination(channelId: channelId)
        }
        .task(id: auth.accessToken) {
            if section.type == .subscriptions,
               let lastSubscriptionAuthToken,
               lastSubscriptionAuthToken != auth.accessToken {
                subscriptionSnapshot = []
                subscriptionTopicsModel.resetVisibleClassification()
            }
            lastSubscriptionAuthToken = auth.accessToken
            await viewModel.updateAuthToken(auth.accessToken)
        }
        .onAppear {
            // TabView keeps each tab alive. History must not reuse a 15-minute-old
            // snapshot when the user returns after watching another video.
            if section.type == .history, viewModel.loadedAt != nil {
                viewModel.reload(section: section)
            } else if section.type == .subscriptions, viewModel.loadedAt != nil {
                viewModel.refreshIfStale()
            }
        }
        .onReceive(NotificationCenter.default.publisher(for: .watchHistoryDidChange)) { note in
            guard section.type == .history,
                  let video = note.userInfo?["video"] as? Video
            else { return }
            viewModel.prependRecentlyWatched(video)
        }
        .onChange(of: scenePhase) { _, phase in
            if phase == .active { viewModel.refreshIfStale() }
        }
        .onChange(of: rawVideos.map(\.id), initial: true) { _, videoIDs in
            guard section.type == .subscriptions, !videoIDs.isEmpty else { return }
            subscriptionSnapshot = rawVideos
        }
        .onChange(of: viewModel.loadedAt) { _, loadedAt in
            guard section.type == .subscriptions,
                  loadedAt != nil,
                  rawVideos.isEmpty,
                  viewModel.error == nil
            else { return }
            subscriptionSnapshot = []
        }
    }

    private var rawVideos: [Video] {
        viewModel.videoGroups
            .flatMap(\.videos)
            .filter { !$0.isShort }
    }

    private var videos: [Video] {
        if section.type == .subscriptions,
           rawVideos.isEmpty,
           !subscriptionSnapshot.isEmpty {
            return subscriptionSnapshot
        }
        return rawVideos
    }

    private var subscriptionContent: some View {
        TVSubscriptionTopicsView(
            videos: videos,
            topicsModel: subscriptionTopicsModel,
            isRefreshing: viewModel.isLoading,
            isLoadingMore: viewModel.isLoadingMore,
            hasMoreContent: viewModel.hasMoreContent,
            lastUpdatedAt: viewModel.loadedAt,
            refreshErrorDescription: viewModel.error?.localizedDescription,
            selectedVideo: $selectedVideo,
            selectedTopicRawValue: $selectedTopicRawValue,
            onRefresh: {
                restoreFocusedVideoID = nil
                viewModel.reload(section: section)
            },
            onLoadMore: {
                guard let last = rawVideos.last ?? videos.last else { return }
                viewModel.loadMoreIfNeeded(lastVideo: last)
            }
        )
    }

    private var standardFeedContent: some View {
        ScrollViewReader { proxy in
            ScrollView {
                VideoGridSection(
                    videos: videos,
                    onSelect: { video in
                        playbackReturnVideoID = video.id
                        restoreFocusedVideoID = nil
                        let capturedVideos = videos
                        Task { @MainActor in
                            await CurrentQueueStore.shared.replaceAll(with: capturedVideos)
                            let startIndex = capturedVideos.firstIndex(where: { $0.id == video.id }) ?? 0
                            selectedVideo = await CurrentQueueStore.shared.videoAt(index: startIndex) ?? video
                        }
                    },
                    loadMore: {
                        if let last = videos.last {
                            viewModel.loadMoreIfNeeded(lastVideo: last)
                        }
                    },
                    restoreFocusedVideoID: restoreFocusedVideoID,
                    onFocusRestored: { videoID in
                        if restoreFocusedVideoID == videoID {
                            restoreFocusedVideoID = nil
                        }
                    }
                )
                if viewModel.isLoading {
                    ProgressView().padding(32)
                }
            }
            .onChange(of: selectedVideo?.id) { previousVideoID, currentVideoID in
                guard previousVideoID != nil,
                      currentVideoID == nil,
                      let videoID = playbackReturnVideoID,
                      videos.contains(where: { $0.id == videoID })
                else { return }

                proxy.scrollTo(videoID, anchor: .center)
                DispatchQueue.main.async {
                    restoreFocusedVideoID = videoID
                }
            }
        }
    }

    private var signedOutState: some View {
        VStack(spacing: 24) {
            Image(systemName: "person.crop.circle.badge.questionmark")
                .font(.system(size: 72))
                .foregroundStyle(.secondary)
            Text("Sign in to see your \(section.title.lowercased())")
                .font(.title2.weight(.semibold))
                .multilineTextAlignment(.center)
            NavigationLink("Open Settings") {
                SettingsView()
            }
                .buttonStyle(.borderedProminent)
                .accessibilityIdentifier("feed.openSettingsButton")
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity)
    }

    private func errorState(_ error: Error) -> some View {
        VStack(spacing: 20) {
            Image(systemName: "wifi.exclamationmark")
                .font(.system(size: 64))
                .foregroundStyle(.secondary)
            Text("Couldn’t load \(section.title)")
                .font(.title2.weight(.semibold))
            Text(error.localizedDescription)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
            Button("Try Again") { viewModel.reload(section: section) }
                .buttonStyle(.borderedProminent)
        }
        .padding(80)
        .frame(maxWidth: .infinity, maxHeight: .infinity)
    }
}
#endif
