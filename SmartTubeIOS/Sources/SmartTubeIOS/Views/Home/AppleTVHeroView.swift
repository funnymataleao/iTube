...
struct AppleTVHeroView: View {
    ...
    private var metadataSection: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            TimelineView(.periodic(from: .now, by: 60)) { context in
                Text(metadataText(relativeTo: context.date))
    ...
    private func metadataText(relativeTo referenceDate: Date) -> String {
        var parts: [String] = []
        ...
        if currentVideo.isUpcoming,
           let publishedText = currentVideo.publishedTimeText,
           !publishedText.isEmpty {
            parts.append(publishedText)
        } else if let publishedLabel = VideoPublishAgeFormatter.label(
            for: currentVideo,
            relativeTo: referenceDate
        ) {
            parts.append(publishedLabel)
        }
